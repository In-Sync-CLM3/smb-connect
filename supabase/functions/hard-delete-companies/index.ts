...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Delegate all DB operations to RPC (single call replaces N*4 sequential roundtrips)
    const { data, error: rpcError } = await supabaseAdmin.rpc('hard_delete_companies', {
